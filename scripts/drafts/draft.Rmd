...
training <- cbind(training_projected, train_labels) 
...
```{r}
best_rf <- select_best(tune_class, "roc_auc")
```
Finalize model
```{r}
...
```{r}
testing <- merge.data.frame(test_projected, test_labels)
predictions <- augment(final_rf_model, testing) %>% 
  select(bg_group, starts_with(".pred"))
...
# Calculate metrics
rf_metrics <- predictions %>% class_metrics(truth = bg_class, 
                  estimate = .pred, 
                  pred, 
                  event_level = 'second')
```