...
This visual represents the number of trips per county by household density categories (urban, suburban, exurban, and rural). One thing to take note of is that the exurban map has far less trips taken than the other maps. The rural map has the most trips taken than the other maps. 
...
Using a confusion matrix, we can see how many predictions the model got right and wrong for each `bg_group` level. We found that the model is best at predicting the Suburban group and the the Urban group. We can also calculate the accuracy metric, which we got 0.443 for this model.
