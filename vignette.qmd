...
We will tune the model on the grid we set up with the cross validation folds. Running a random forest model can take a long time, so we will save our tune results to an RData file to prevent continuously run the model tuning.
...
Using a confusion matrix, we can see how many predictions the model classified correctly and incorrectly for each `bg_group` level. We found that the model is best at predicting the Suburban group and the Urban group. After calculating the metrics for this model, we found that it had an accuracy of 0.447.
...
We also want to take a look at the top 10 most influential variables to the model's predictions. The most important variable was the total number of miles the person traveled on the survey data, followed by age, and then the number of trips the household takes.
