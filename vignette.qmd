...
county_bg_aggreg <- personHHData %>% 
  group_by(County, CTFIP, bg_group) %>% 
  mutate(count = n()) %>% 
...
```{r}
# PC1 vs PC2 with class separation
ggplot(pca_df, aes(x = PC1, y = PC2, color = bg_group)) +
  geom_point(size = 2) +
  labs(title = "PC1 vs PC2 Colored by bg_group",
       x = "Principal Component 1",
       y = "Principal Component 2") +
  theme_minimal()
```

This plot illustrates the reduced feature space using the first two principal components. We see a clear overlap between classes and non-linear relationships, which means that the linear combination of features through PCA does not separate classes in a clear and simple way. This suggests that PCA may not fully capture the complexities of the data, making it challenging to use linear models like logistic regression to classify the data accurately.
...
Random Forest is an ensemble learning method that combines the predictions of multiple individual decision trees to make a final prediction. It is a flexible and powerful algorithm with the idea of building a large number of decision trees during training and then aggregating their predictions to improve accuracy and reduce overfitting.

For our data, random forest model is a good choice because they are able to model complex, non-linear relationships between features and the predictor. They are also effective even in high-dimensional spaces and can handle large numbers of features, where the most relevant features are automatically focused on when building the trees.

### Step 1: Set up the data for Random Forest modelling

For Random Forest modelling, we want to use the original clean dataset, from before preprocessing for PCA. However, there is a lot of missing data that we need to take a look at:
...
From the table, you will see there is a high amount of missing data in `DisLicensePlt` (if the respondent has a disabled license plate), `SchoolMode` (how the respondent travels to school), and WorkMode (how the respondent travels to work). We will impute these missing values with 'Not Disabled', 'Not in School', and 'Not Working' respectively. We will also remove any location related variables, as those are highly correlated to `bg_group` and remove any other rows with missing values.
...
Now, we want to specify what kind of model we want to run with `rand_forest()`. We want to set the parameters, `mtry`, `trees`, and `min_n`, to `tune()`, so that we can later use cross-validation to test which values is the most optimal to set the parameters to. We will use the `randomForest` engine and set the mode to classification.
...
Now that we looked through the different parameter combinations, we want to extract the parameters that led to the most accurate model and finalize the model with those parameters. The best parameters can be seen below:
