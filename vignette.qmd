...
-   Building a random forest model using cross-validation
...
Now that we have concluded that this dataset fails the linearity assumption, we know that it would be best modeled by other classification models, which is what we will fit next.
...
save(tune_results, file = "rfmodels.rda")
```
